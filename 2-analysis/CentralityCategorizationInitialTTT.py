...
# Toma las centralidades calculadas para cada jugador en Centralities.py, y las categoriza en tres grupos: low, medium y high. Cada uno de estos grupos se refiere al valor de centralidad que tuvo el jugador, tomando algun criterio para resumir la informacion, dado que cada jugador tiene asociada una lista con sus correspondientes valores de centralidad a lo largo del tiempo.
#
...
#
# Este script genera un pickle para cada algoritmo de centralidad, conteniendo un diccionario con keys [low, medium, high], y los jugadores correspondientes en cada grupo.
# Tambien genera un grafico de (valor de centralidad vs. habilidad) para cada centralidad, dibujando las curvas para cada una de los 3 grupos de centralidad.
...
def main():
    ...
    centrality_groups = pickle.load(open('data/enddate/150-games-centrality-categorization-value-groups.pickle', 'rb'))
    for centrality in CENTRALITY_NAMES:#centrality="degree-centrality"
        ...
        with open('data/enddate/{}-{}-ttt-{}-50-games-categorization-groups.pickle'.format(get_summary(), centrality, ttt_level), 'wb') as handle:
            pickle.dump(
        ...
        fig.savefig("imagenes/{}/evolucion-skill-centralities-ttt/evolucion-{}-{}.pdf".format(game_order_folder,get_file_name(),centrality),bbox_inches="tight", pad_inches=0)
        plt.close(fig)
        nombre = "imagenes/{}/evolucion-skill-centralities-ttt/evolucion-{}-{}.pdf".format(game_order_folder,get_file_name(),centrality)
